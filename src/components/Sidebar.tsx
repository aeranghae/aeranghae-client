...
const Sidebar: React.FC<SidebarProps> = ({ activeMenu, setActiveMenu }) => {
  ...
  // 로그아웃 (새로고침 시 App.tsx가 로그인 페이지를 띄움)
  const handleLogout = () => {
  ...
  // 게스트가 로그인 창으로 돌아가고 싶을 때
  const handleGoToLogin = () => {
    localStorage.removeItem('aeranghae_token');
    localStorage.removeItem('aeranghae_user_name');
    window.location.reload(); // App.tsx의 !token 조건에 걸려 로그인창 등장
  };

  return (
    <aside className="w-64 h-full relative flex flex-col border-r border-white/10 bg-black/20 backdrop-blur-xl pt-8 z-20 shrink-0 select-none">
      ...
      <div className="p-4 border-t border-white/10">
        <div className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${token ? 'bg-white/5 border-white/5' : 'bg-blue-600/5 border-blue-500/20'}`}>
          ...
          {token ? (
            <button onClick={handleLogout} className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all active:scale-90" title="Logout">
              <LogOut size={16} />
            </button>
          ) : (
            <button onClick={handleGoToLogin} className="p-1.5 text-blue-400 hover:bg-blue-400/10 rounded-lg transition-all active:scale-90 flex flex-col items-center gap-0.5" title="Login">
